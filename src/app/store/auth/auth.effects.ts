...
export class AuthEffects {
  ...
  loginAttempt = this.actions$.pipe(
    ...
    exhaustMap((actionPayload: Auth.LoginAttempt) => {
      const username = actionPayload.username;
      const password = actionPayload.password;
      return from(
        this.afa.auth.signInWithEmailAndPassword(username, password)
      ).pipe(
        map(result => {
  ...
  @Effect()
  signupAttempt = this.actions$.pipe(
    ofType(Auth.AuthActionTypes.SIGN_UP),
    switchMap((actionPayload: Auth.SignUp) => {
  ...
  loginAttemptFail = this.actions$.pipe(
  ...
  signupFail = this.actions$.pipe(
    ...
    tap(() => {
      ...
      this.uiService.openSnackBar(
        'there was a problem registering the account',
        null,
        5000
      );
    })
